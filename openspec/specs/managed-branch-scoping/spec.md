...
Ensures a Manager's permission grants on subShift-linked subjects (`assignments`, `availability`)
are scoped to the branches they actually manage, using the same relation-chain condition pattern
already applied to `sub-shifts` and `tasks` — so a Manager's access never silently spans every
branch in the system, only the ones they're responsible for.
