...
Lets a Manager see every employee's registered availability for branches they manage — via the
same `GET /availability` endpoint an Employee uses, scoped by their permission grant rather than
role-specific branching — so the manager-facing roster-building screen has a real data source to
build a week's shifts from. Manager's access is read-only: the only way availability turns into an
actual shift is `POST /assignments`, which flips `Availability.status` to `ASSIGNED` internally.
