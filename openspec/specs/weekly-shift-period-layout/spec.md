# weekly-shift-period-layout Specification

## Purpose

A shared, reusable weekly schedule grid (period bands + spanning shift blocks) used consistently
across every schedule-shaped Staff screen, so Bản biểu, Đăng ban, and future tabs like Bản ký all
render shifts the same way instead of each screen inventing its own layout.
